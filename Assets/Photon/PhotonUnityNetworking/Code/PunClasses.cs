...
namespace Photon.Pun
{
    ...
    public class DefaultPool : IPunPrefabPool
    {
        ...
        {
            GameObject res = null;
            bool cached = this.ResourceCache.TryGetValue(prefab.name, out res);
            if (!cached)
            {
                res = (GameObject)prefab;
                if (res == null)
                {
                    Debug.LogError("DefaultPool failed to load \"" + prefab + "\" . Make sure it's in a \"Resources\" folder.");
                }
                else
                {
                    this.ResourceCache.Add(prefab.name, res);
                }
            }
